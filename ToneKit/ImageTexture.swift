...
/// Image source used for image processing. On initialization, the specified UIImage is loaded as a
/// MTLTexture.
open class ImageTexture: TextureOutput {
  /// Default texture options for loading a image with MTLTextureLoader.
  @available(iOS 10.0, *)
  ...
  public let metalDevice: MetalDevice
  ...
  ///
  /// - Parameters:
  ///   - metalDevice: MetalDevice containing the processing queue.
  ///   - image: Image to load.
  ///   - options: MTLTextureLoader options.
  public init(metalDevice: MetalDevice = MetalDevice.shared,
              image: UIImage,
              options: [MTKTextureLoader.Option: Any] = ImageTexture.defaultOptions) {
    ...
    let textureLoader = MTKTextureLoader(device: self.metalDevice.device)
    do {
  ...
  open func process() {
    ...
    metalDevice.processingQueue.async {
...
extension ImageTexture {
  /// Helper function to fix the image orientation of a UIImage since the orientation of the UIImage
  /// may sometimes be different from its CGImage counterpart.
  ///
  /// - Parameters:
  ///   - image: Image to fix.
