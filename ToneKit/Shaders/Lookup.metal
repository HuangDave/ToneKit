...
kernel void compute_lookup(texture2d<float, access::sample> texture_in     [[texture(0)]],
                           texture2d<float, access::sample> lookup_texture [[texture(1)]],
                           texture2d<float, access::write>  texture_out    [[texture(2)]],
                           ...
                           uint2 gid [[thread_position_in_grid]])
{
    ...
    float2 texture_position1 = float2((quad1.x * 0.125) + 0.5/512.0 + ((0.125 - 1.0/512.0) * color_in.r),
                                      (quad1.y * 0.125) + 0.5/512.0 + ((0.125 - 1.0/512.0) * color_in.g));
    float2 texture_position2 = float2((quad2.x * 0.125) + 0.5/512.0 + ((0.125 - 1.0/512.0) * color_in.r),
    ...
    float4 color = mix(new_color1,
                       new_color2,
                       fract(blue));

    texture_out.write(mix(color_in, float4(color.rgb, color_in.a), intensity), gid);
}
