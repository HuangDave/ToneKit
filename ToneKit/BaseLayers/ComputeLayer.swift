...
open class ComputeLayer: TextureIO {
    ...
    open var threadgroups: MTLSize {
        return MTLSize(width:  Int(floorf(Float(outputSize.width)  / Float(threadgroupCount.width))),
                       height: Int(floorf(Float(outputSize.height) / Float(threadgroupCount.height))),
                       depth:  1)
    }
    ...
    ///
    /// Should override to set any required uniforms.
