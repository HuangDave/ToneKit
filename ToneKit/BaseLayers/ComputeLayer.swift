...
open class ComputeLayer: TextureIO {
  open var functionName: String { return "ComputePassthrough" }
  open var inputCount: UInt { return 1 }
  internal let metalDevice: MetalDevice
  /// Array containing all input textures that are used for processing. The size allocated for the
  /// array is based on the specified inputCount.
  ...
  public internal(set) var outputSize: MTLSize = MTLSize(width: 0, height: 0, depth: 0)
  /// This value is set to false when processingDidFinish() is completed.
  public internal(set) var isProcessing: Bool = false
  /// Set to true if the layer needs processing.
  public var isDirty: Bool = true
  /// If false, then the layer will not be processed and the incoming texture will be passed
  ...
  ///
  /// ie. The default threadgroupCount is 8 x 8. If the texture size is 32 x 48, then the
  ///     threadgroupSize is 4 x 6.
  ///
  ...
  public init(metalDevice: MetalDevice = MetalDevice.shared) {
    ...
    computeFunction = metalDevice.makeFunction(name: functionName)
    do {
      try computePipeline = metalDevice.device.makeComputePipelineState(function: computeFunction)
    } catch {
      fatalError(
        "Error occurred when building compute pipeline for function: \(computeFunction.name)")
    }
  ...
  ///
  /// Uniforms are automatically encoded to the MTLComputeCommandEncoder in the order they are
  /// registered.
  ///
  ...
  open func generateOutputTextureIfNeeded() {
    if outputTexture == nil ||
      ...
      outputTexture = metalDevice.device.makeTexture(descriptor: textureDescriptor)
    }
  }
  ...
  ///
  /// - Returns: True if all checks pass or if all pre-processing tasks are completed, otherwise
  ///            return false and image processing is terminated.
...
extension ComputeLayer: TextureInput {
  ...
  ///   - index:   Index of texture to update if the layer utilizes multiple inputs. By default
  ///              index = 0.
